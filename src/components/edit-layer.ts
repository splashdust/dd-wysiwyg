...
export class EdEditLayer extends LitElement {
  ...
  buildFromElement(element: EdElement) {
    if (!element.renderedElement) return;
      ...
      ${this.overlays.map(
        (overlay) => html`
          <div
            ...
            style="
                            top: ${overlay.elementRef.getBoundingClientRect()
              .top}px;
                            left: ${overlay.elementRef.getBoundingClientRect()
              .left}px;
                            width: ${overlay.elementRef.getBoundingClientRect()
              .width}px;
                            height: ${overlay.elementRef.getBoundingClientRect()
              .height}px;
                        "
          >
            <div class="tag-name">
              &lt;${overlay.elementRef.tagName.toLowerCase()}&gt;
            </div>
            ${overlay.layout}
          </div>
        `
